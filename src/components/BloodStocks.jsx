  ...
  return (
    ...
    >
      ...
      <div className={`mt-4 pt-4 flex justify-between items-center ${
        ...
        <button
          onClick={() => { if (status !== 'ok') scrollTo('devenir-donneur'); }}
...
const BloodStocks = () => {
 ...
 useEffect(() => {
  ...
  return (
    <section id="stocks" className="w-full py-16 bg-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-slate-400 text-sm max-w-xl">
              Les niveaux sont affichés sous forme d'indicateurs. Pour toute
              demande, contactez directement le service de transfusion du CNHU-HKM.
            </p>
          </div>
            ...
            {[
              ...
              <button key={key} onClick={() => setFilter(key)}
                className={`px-5 py-2 rounded-full text-xs font-bold transition-all ${
                  filter === key ? 'bg-white text-red-700 shadow' : 'text-white/60 hover:text-white'
                }`}>
                {label}
              </button>
            ))}
        ...
        <div className="mt-8 p-5 bg-white/5 border border-white/10 rounded-2xl flex flex-col md:flex-row items-start md:items-center gap-4">
          ...
          <p className="text-slate-400 text-xs leading-relaxed">
            <strong className="text-white">Note importante :</strong> Les niveaux
            affichés sont indicatifs. Toute demande de sang doit être formulée par
            un professionnel de santé du CNHU-HKM. Pour une urgence médicale,
            appelez directement le service de transfusion.
          </p>
          <a href="tel:+22921301045"
